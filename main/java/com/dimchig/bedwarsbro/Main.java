...
public class Main {
    ...
    private void initConfig() {
        ...
        prop = config.get(Configuration.CATEGORY_CLIENT, CONFIG_MSG.AUTO_EJECTION.text, false,                        ColorCodesManager.replaceColorCodesInString("&fМод автоматически выбросит такие ресурсы как - &7Железо&f, &eЗолото&f, &bАлмазы &fи &aИзумруды."));
        ...
        prop = config.get(Configuration.CATEGORY_CLIENT, CONFIG_MSG.CORRECT_FIREBALL.text, false,                        ColorCodesManager.replaceColorCodesInString("&fКорректирует &cфаербол &fчтобы летел идеально туда куда вы смотрите."));
